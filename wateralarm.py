...
#GPIO.setup(GPIO14, GPIO.OUT, initial=GPIO.LOW)        #Sætter pin som output, og starter alarmerne slukket
...
class main:                                           #Class for at bruge flere variabler i én funktion
    def checkroomtemp():
        roomtemp = 0
        #schedule.every(60).minutes.do(roomtemp)
           # while True:
        ...
        while True:
            if roomtemp is not None:
                roomtemp = num_2
                print(roomtemp, (now.strftime("%d-%m-%Y %H:%M:%S")))
                return num_2
    ...
    def checktemp(a):                                  #Funktionen med alt indmaden
            ...
            if watertemp is not None:
                ...
                print(watertemp, (now.strftime("%d-%m-%Y %H:%M:%S")))
                if watertemp > 28:                           #Hvis temperaturen er OVER angivet værdi, så går alarmen af
                    GPIO.output(GPIO14, True)
                    GPIO.output(GPIO26, True)
                elif watertemp < 20:                         #Hvis temperaturen er UNDER angivet værdi, så går alarmen af
                    GPIO.output(GPIO14, True)
                    GPIO.output(GPIO26, True)
